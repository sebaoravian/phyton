...
from data_processing import cargar_datos_rango_fechas as cargar_datos  # Actualizar la importación
...
def main():
    ...
    with st.sidebar:
        ...
        rango_fechas = st.selectbox("Seleccionar Rango de Fechas", ["Mes en curso", "Mes anterior", "Últimos 30 días", "Últimos 60 días", "Últimos 90 días", "Últimos 7 días", "Semana anterior", "Personalizado"])
        ...
        df_checkins = cargar_datos(filtros["fecha_inicio_manual"], filtros["fecha_fin_manual"], "detailscheckins.csv", "split_checkins")
        
        if not df_checkins.empty:
            user_area_name_options = df_checkins["user_area_name"].dropna().unique() if "user_area_name" in df_checkins.columns else []
            user_role_name_options = df_checkins["user_role_name"].dropna().unique() if "user_role_name" in df_checkins.columns else []
            user_region_name_options = df_checkins["user_region_name"].dropna().unique() if "user_region_name" in df_checkins.columns else []
            user_department_name_options = df_checkins["user_department_name"].dropna().unique() if "user_department_name" in df_checkins.columns else []
            pos_area_name_options = df_checkins["pos_area_name"].dropna().unique() if "pos_area_name" in df_checkins.columns else []
            pos_format_name_options = df_checkins["pos_format_name"].dropna().unique() if "pos_format_name" in df_checkins.columns else []
            pos_cluster_name_options = df_checkins["pos_cluster_name"].dropna().unique() if "pos_cluster_name" in df_checkins.columns else []
            pos_retailer_name_options = df_checkins["pos_retailer_name"].dropna().unique() if "pos_retailer_name" in df_checkins.columns else []
            pos_region_name_options = df_checkins["pos_region_name"].dropna().unique() if "pos_region_name" in df_checkins.columns else []
            pos_flag_name_options = df_checkins["pos_flag_name"].dropna().unique() if "pos_flag_name" in df_checkins.columns else []
