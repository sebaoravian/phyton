...
import pandas as pd
from data_processing import cargar_datos_rango_fechas, limpiar_identificadores, limpiar_porcentajes, limpiar_valores
...
def listado_formularios(filtros):
    ...
    df_forms = cargar_datos_rango_fechas(filtros["fecha_inicio_manual"], filtros["fecha_fin_manual"], "detailsforms.csv", "split_forms")
    ...
    df_checkins = cargar_datos_rango_fechas(filtros["fecha_inicio_manual"], filtros["fecha_fin_manual"], "detailscheckins.csv", "split_checkins")
    
    if df_checkins.empty:
        st.warning("No se encontraron datos de check-ins para la fecha actual.")
        return
    
    df_checkins = limpiar_identificadores(df_checkins)
    
    if "id_checkin" in df_forms.columns and "checkin_id" in df_checkins.columns:
    ...
    # Verificar si la columna 'name_activity' existe antes de intentar acceder a ella
    if "name_activity" in df_forms.columns:
    ...
    df_forms_anterior = cargar_datos_rango_fechas(fecha_anterior, fecha_anterior, "detailsforms.csv", "split_forms")
    
    if df_forms_anterior.empty:
    ...
    if "id_checkin" in df_forms_anterior.columns and "checkin_id" in df_checkins.columns:
    ...
    actividades = df_forms["name_activity"].unique() if "name_activity" in df_forms.columns else []
    ...
    for i, actividad in enumerate(actividades_en_orden):
        actividad_df = df_forms[df_forms["name_activity"] == actividad] if "name_activity" in df_forms.columns else pd.DataFrame()
        conteo = actividad_df.shape[0]
        conteo_anterior = df_forms_anterior[df_forms_anterior["name_activity"] == actividad].shape[0] if "name_activity" in df_forms_anterior.columns else 0
        ...
        if actividad in actividades_prioritarias:
            ...
            promedio_ponderation_anterior = df_forms_anterior[df_forms_anterior["name_activity"] == actividad]["ponderation"].mean() if "ponderation" in df_forms_anterior.columns else 0
